...
// ✅ إصلاح: import من المصدر الموحد بدل تعريف محلي
...
export default function AdminUsersPage() {
  ...
  const confirmAction = async () => {
    ...
    try {
      if (type === "ban") {
        ...
        // ✅ إصلاح: نقرأ trustLevel الفعلي من الـ response بدل تثبيت 2
        const res = await axiosInstance.post(`/api/admin/users/${userId}/promote`, {
          reason:    note || "ترقية يدوية من الأدمن",
          adminNote: note || null,
        });
        const newLevel = res.data.user?.trustLevel ?? 2;
        showToast(`✅ تمت ترقية المستخدم إلى المستوى ${newLevel} بنجاح`, true);
        setUsers((prev) =>
          prev.map((u) =>
            u._id === userId ? { ...u, trustLevel: newLevel } : u
          )
        );

      } else if (type === "demote") {
        // ✅ إصلاح: نقرأ trustLevel الفعلي من الـ response بدل تثبيت 1
        const res = await axiosInstance.post(`/api/admin/users/${userId}/demote`, {
          reason:    note || "تخفيض يدوي من الأدمن",
          adminNote: note || null,
        });
        const newLevel = res.data.user?.trustLevel ?? 1;
        showToast(`✅ تم تخفيض المستخدم إلى المستوى ${newLevel} بنجاح`, true);
        setUsers((prev) =>
          prev.map((u) =>
            u._id === userId ? { ...u, trustLevel: newLevel } : u
          )
  ...
  // ✅ إصلاح: labels ديناميكية — لا نذكر رقم مستوى محدد في الـ title
  const ACTION_LABELS: Record<PendingAction["type"], { title: string; btn: string; btnColor: string }> = {
    ...
    demote:  { title: "تأكيد التخفيض",     btn: "تخفيض للمستوى 1", btnColor: "bg-orange-500 hover:bg-orange-600 text-white" },
  };
        ...
        >
          ...
          >
            ...
            <p className="text-[13px] text-gray-500">
              المستخدم:{" "}
              <span className="font-bold text-gray-800">{pending.userName}</span>
            </p>
            ...
            <div className="flex gap-2 pt-1">
              <button
                ...
                className={`flex-1 py-2.5 rounded-xl text-[13px] font-black transition-all ${ACTION_LABELS[pending.type].btnColor}`}
              >
                  ...
                  >
                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-1.5">
                        {/* ✅ إصلاح: badge ديناميكي يعرض المستوى الحقيقي */}
                        <span
                        ...
                        {/* ✅ إصلاح: زر الترقية يظهر فقط إذا كان المستوى 1 (promote يدوي للـ 2 فقط) */}
                        {u.trustLevel === 1 && (
                          <button
                            onClick={() => openConfirm(u, "promote")}
                            disabled={busy[u._id]}
                            title="ترقية للمستوى 2"
                            className="w-6 h-6 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-100 disabled:opacity-40 transition-all flex items-center justify-center text-[10px] font-black"
                          >
                            ▲
                          </button>
                        )}

                        {/* ✅ إصلاح: زر التخفيض يظهر عند أي مستوى أعلى من 1 */}
                        {u.trustLevel > 1 && (
                          <button
                            onClick={() => openConfirm(u, "demote")}
                            disabled={busy[u._id]}
                            title="تخفيض للمستوى 1"
                            className="w-6 h-6 rounded-lg bg-orange-50 text-orange-600 hover:bg-orange-100 disabled:opacity-40 transition-all flex items-center justify-center text-[10px] font-black"
                          >
                            ▼
                          </button>
                        )}
                    ...
                    <td className="p-4">
                      <button
                        ...
                        disabled={busy[u._id]}
                        className={`px-3 py-1.5 rounded-lg text-[11px] font-black disabled:opacity-40 transition-all ${
                          u.isBanned
