...
export default function AdminUsersPage() {
  ...
  const confirmAction = async () => {
    ...
    try {
      if (type === "ban") {
        ...
        showToast("🚫 تم حظر المستخدم", true);
        setUsers((prev) =>
        ...
        setUsers((prev) =>
          prev.map((u) => (u._id === userId ? { ...u, trustLevel: 2 } : u))
        );
      } else if (type === "demote") {
        ...
        setUsers((prev) =>
          prev.map((u) => (u._id === userId ? { ...u, trustLevel: 1 } : u))
        );
      }
    } catch (err: unknown) {
  ...
  const ACTION_LABELS: Record<
    PendingAction["type"],
    { title: string; btn: string; btnColor: string }
  > = {
    ban: {
      title: "تأكيد الحظر",
      btn: "حظر المستخدم",
              ...
              users.map((u) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="p-4">
                      <button
                        onClick={() => openConfirm(u, u.isBanned ? "unban" : "ban")}
                        disabled={busy[u._id]}
