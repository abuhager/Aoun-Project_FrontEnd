...
export default function DashboardPage() {
  ...
  const handleReport = async (userId: string, itemTitle: string) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await axios.post(`${backendBaseUrl}/api/items/report-user`, 
  ...
  const handleRateItem = async () => {
    ...
    try {
      ...
      const token = localStorage.getItem("token");
      const res = await axios.put(`${backendBaseUrl}/api/items/rate/${selectedItem._id}`, 
        { rating },
        { headers: { "x-auth-token": token } }
      );
      alert(res.data.msg);
      ...
      <main className="pt-20 md:pt-24 px-4 md:px-8 max-w-6xl mx-auto space-y-6 md:space-y-8">
        ...
        <section className="bg-white rounded-2xl p-6 md:p-8 text-center flex flex-col items-center gap-3 shadow-sm border border-[#edeeef] relative overflow-hidden">
          ...
          <div className="w-20 h-20 md:w-24 md:h-24 rounded-full bg-slate-100 flex items-center justify-center ring-2 ring-primary/10 z-10">
            <span className="material-symbols-outlined text-4xl md:text-5xl text-primary">account_circle</span>
          </div>
          <div className="z-10 flex flex-col items-center">
            <h1 className="text-xl md:text-2xl font-black">{data.user?.name || "مستخدم عون"}</h1>
            <p className="text-xs md:text-sm text-on-surface-variant mt-1">{data.user?.email}</p>
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
          <div className="bg-white p-5 rounded-2xl shadow-sm border border-[#edeeef] text-center flex flex-col items-center justify-center">
            ...
            <div className="relative w-20 h-20 mx-auto flex items-center justify-center">
              <svg className="w-full h-full -rotate-90">
                ...
                <circle className="text-primary" cx="40" cy="40" r="36" fill="transparent" strokeWidth="6" strokeDasharray="226" strokeDashoffset={226 - (226 * (data.user?.trustScore || 85)) / 100}></circle>
              </svg>
          ...
          <div className="bg-white p-5 rounded-2xl shadow-sm border border-[#edeeef] flex flex-col justify-center text-center gap-3">
            ...
            <div className="h-2 w-full bg-[#e7e8e9] rounded-full overflow-hidden">
              <div className="h-full bg-blue-500 rounded-full transition-all duration-500" style={{ width: `${((data.user?.quota || 0) / 3) * 100}%` }}></div>
            </div>
            <div className="bg-blue-50 text-blue-700 px-3 py-1.5 rounded-xl text-[10px] font-bold">
              {(data.user?.quota ?? 3) > 0 ? `متاح لك حجز ${data.user?.quota ?? 3} أغراض إضافية` : "لقد استنفدت حصتك لهذا الأسبوع ⚠️"}
            </div>
          </div>
        ...
        <section className="space-y-4">
          <div className="flex border-b border-[#edeeef] gap-4 overflow-x-auto">
            <button onClick={() => setActiveTab("donations")} className={`px-4 py-3 font-bold text-sm transition-all ${activeTab === "donations" ? "text-primary border-b-2 border-primary" : "text-on-surface-variant"}`}>تبرعاتي</button>
            <button onClick={() => setActiveTab("requests")} className={`px-4 py-3 font-bold text-sm transition-all ${activeTab === "requests" ? "text-primary border-b-2 border-primary" : "text-on-surface-variant"}`}>طلباتي</button>
          </div>
                    ...
                    <tr key={item._id} className="hover:bg-surface transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <div className="relative w-10 h-10 overflow-hidden rounded-lg">
                          <Image 
                            alt={item.title} 
                            src={item.imageUrl ? (item.imageUrl.startsWith("http") ? item.imageUrl : `${backendBaseUrl}/${item.imageUrl}`) : "https://via.placeholder.com/100"} 
                            fill
                            className="object-cover"
                          />
                        </div>
                      </td>
                      <td className="px-4 py-3 text-on-surface-variant">{new Date(item.createdAt).toLocaleDateString("ar-EG")}</td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-center gap-2">
                          <button onClick={() => router.push(`/items/${item._id}`)} className="text-primary hover:underline font-bold text-xs">التفاصيل</button>
                          
                          {activeTab === "requests" && item.status === "محجوز" && item.otp && (
                            <div className="flex items-center gap-1.5 bg-blue-50 px-2.5 py-1.5 rounded-lg border border-blue-100 cursor-help" title="أعطِ هذا الرمز للمتبرع عند الاستلام">
                          ...
                          {activeTab === "donations" && item.status === "متاح" && (
                            <button onClick={() => handleDelete(item._id)} className="text-red-500 hover:bg-red-50 p-1.5 rounded-lg"><span className="material-symbols-outlined text-sm">delete</span></button>
                          )}

                          {activeTab === "donations" && item.status === "محجوز" && (
                            <button onClick={() => { setSelectedItem(item); setShowModal(true); }} className="bg-primary text-white px-3 py-1 rounded-lg text-[10px] font-bold">تأكيد التسليم</button>
                          )}

                          {activeTab === "requests" && item.status === "تم التسليم" && !item.isRated && (
                            <button onClick={() => { setSelectedItem(item); setShowRatingModal(true); }} className="bg-yellow-100 text-yellow-700 px-3 py-1 rounded-lg text-[10px] font-bold flex items-center gap-1 hover:bg-yellow-200">
                               <span className="material-symbols-outlined text-xs">star</span> تقييم
                            </button>
                          )}

                          {(item.status === "محجوز" || item.status === "تم التسليم") && (
                            <>
                              <a href={`https://wa.me/${(activeTab === "requests" ? item.donor?.phone : item.bookedBy?.phone)?.replace(/\D/g, '')}`} target="_blank" className="bg-[#25D366] text-white p-1.5 rounded-lg">
        ...
        <div className="fixed inset-0 z-110 flex items-center justify-center bg-black/50 backdrop-blur-sm px-4">
          <div className="bg-white rounded-3xl p-8 max-w-sm w-full shadow-2xl text-center">
            <div className="flex justify-center gap-2 mb-6">
              {[1, 2, 3, 4, 5].map((star) => (
                <button key={star} onClick={() => setRating(star)} className={`transition-all hover:scale-125 ${rating >= star ? 'text-yellow-400' : 'text-gray-300'}`}>
                  <span className="material-symbols-outlined text-4xl" style={{ fontVariationSettings: `'FILL' ${rating >= star ? 1 : 0}` }}>star</span>
                </button>
              ))}
            </div>
            <h3 className="text-xl font-bold mb-6 text-[#191c1d]">قيم تجربتك مع {activeTab === "requests" ? selectedItem.donor?.name : selectedItem.bookedBy?.name}</h3>
            <div className="flex flex-col gap-3">
              <button onClick={handleRateItem} disabled={ratingLoading || rating === 0} className="w-full bg-primary text-white font-bold py-4 rounded-2xl shadow-lg disabled:opacity-50">{ratingLoading ? "جاري الحفظ..." : "إرسال التقييم"}</button>
              <button onClick={() => { setShowRatingModal(false); setRating(0); }} className="w-full text-on-surface-variant font-bold py-2 text-sm">إلغاء</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showModal && selectedItem && (
        <div className="fixed inset-0 z-100 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
          <div className="bg-white rounded-2xl p-6 max-w-sm w-full shadow-2xl text-center">
            ...
            <p className="text-on-surface-variant text-xs mb-5">أدخل رمز الـ OTP الخاص بـ <span className="font-bold">{selectedItem.title}</span></p>
            ...
            <form onSubmit={handleConfirmDelivery}>
              <input type="text" maxLength={4} className="w-full bg-surface-container-highest text-center text-2xl font-black py-3 rounded-xl mb-5 outline-none" placeholder="••••" value={otp} onChange={(e) => setOtp(e.target.value)} required />
              <div className="flex gap-2">
                <button type="submit" disabled={otpLoading || otp.length !== 4} className="flex-1 bg-primary text-white text-sm font-bold py-3 rounded-full">{otpLoading ? "جاري التأكيد..." : "تأكيد"}</button>
                <button type="button" onClick={() => setShowModal(false)} className="flex-1 bg-surface-container-low text-on-surface-variant text-sm font-bold py-3 rounded-full">إلغاء</button>
              </div>
            </form>
          </div>
        </div>
