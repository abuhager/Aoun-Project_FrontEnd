...
export default function BrowsePage() {
  ...
  const [currentPage, setCurrentPage] = useState(1); // ضفنا هاي بالمرة عشان رح تلزمنا هسا بالتنقل بين الصفحات
  ...
  useEffect(() => {
    ...
    const fetchItems = async () => {
      try {
        ...
        // ✅ بتقدر كمان تمرر الصفحة الحالية لو حابب، مثلاً: ?page=1&limit=12
        const res = await axios.get(`${apiUrl}/api/items`);

        if (isMounted) {
          setItems(res.data.items || []); // الوصول للمصفوفة

          // ✅ الخطأ كان هون، لازم نعطيها المصفوفة كمان مش الكائن كامل
          setFilteredItems(res.data.items || []);

          // (تأكد إنك معرف setTotalPages فوق بالـ state عشان ما يضرب إيرور)
            ...
            {filteredItems.map((item) => (
              ...
              >
                ...
                <div className="p-6">
                  ...
                  <Link
                    href={`/items/${item._id}`}
                    className="w-full py-3 bg-primary text-white rounded-full font-bold flex items-center justify-center gap-2 hover:bg-primary-container hover:shadow-lg transition-all active:scale-95"
                  >
