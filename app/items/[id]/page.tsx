...
export default function ItemDetailsPage() {
  ...
  const isWaitlisted = item?.waitlist?.some(
    (w) => getUserId(w.user) === currentUserId,
  );
  ...
  const handleRequestItem = async () => {
    ...
    try {
      ...
      if (axios.isAxiosError(err)) {
        setMessage({
          type: "error",
          text: err.response?.data?.msg || "حدث خطأ أثناء الطلب",
        });
      } else {
  ...
  const handleCancelAction = async () => {
    ...
    try {
      setActionLoading(true);
      const res = await axios.put(
        `${backendUrl}/api/items/cancel/${id}`,
        {},
        { headers: { "x-auth-token": token } },
      );
      setMessage({ type: "success", text: res.data.msg });
      ...
      if (axios.isAxiosError(err)) {
        setMessage({
          type: "error",
          text: err.response?.data?.msg || "حدث خطأ أثناء العملية",
        });
      } else {
        setMessage({ type: "error", text: "حدث خطأ غير متوقع" });
      }
  ...
  if (!item)
    return (
      <div className="text-center py-20 font-bold">🛑 القطعة غير موجودة</div>
    );

  return (
    <div
      className="bg-surface min-h-screen text-[#191c1d] font-body pb-20"
      dir="rtl"
    >
      <Navbar />
      <main className="pt-20 md:pt-24 px-4 md:px-8 max-w-5xl mx-auto">
        <nav className="mb-6 flex items-center gap-2 text-on-surface-variant text-xs md:text-sm font-medium">
          ...
          <span className="material-symbols-outlined text-[10px] md:text-xs text-gray-400">
            chevron_left
          </span>
          <span className="text-[#191c1d] font-bold truncate max-w-37.5 md:max-w-xs">
            {item.title}
          </span>
        </nav>
          ...
          <div className="flex flex-col gap-6">
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                <span className="px-3 py-1 rounded-lg bg-gray-100 text-gray-600 text-[10px] font-bold border border-gray-200">
                  {item.category}
                </span>
                {item.condition && (
                  <span className="px-3 py-1 rounded-lg bg-primary/5 text-primary text-[10px] font-bold border border-primary/10">
                    {item.condition}
                  </span>
                )}
              </div>
              <h1 className="text-3xl md:text-4xl font-black text-[#191c1d] font-headline leading-tight">
                {item.title}
              </h1>
              <p className="text-sm md:text-base text-on-surface-variant leading-relaxed bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
                {item.description}
              </p>
            </div>
            ...
            {item.status === "محجوز" && item.waitlist?.length > 0 && (
              <div className="flex items-center gap-2 p-3 bg-orange-50 border border-orange-100 rounded-xl">
                ...
                <p className="text-xs font-bold text-orange-700">
                  هناك {item.waitlist.length} طلاب في قائمة الانتظار لهذا الغرض.
                </p>
              </div>
            )}
            {item.status === "محجوز" && (
              <div className="flex items-center gap-2 p-3 bg-blue-50 border border-blue-100 rounded-xl">
                ...
                <p className="text-xs font-bold text-blue-700">
                  هذا الغرض محجوز. إذا لم يتم الاستلام خلال 72 ساعة سيعود متاحاً
                  تلقائياً.
                </p>
              </div>
            )}
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                {
                  label: "التاريخ",
                  val: new Date(item.createdAt).toLocaleDateString("ar-EG"),
                  ic: "event",
                },
                {
                  label: "الموثوقية",
                  val: item.donor?.trustScore + "%",
                  ic: "verified_user",
                },
              ].map((s, i) => (
                <div
                  key={i}
                  className="bg-white p-3 rounded-2xl border border-gray-100 text-center shadow-sm"
                >
                  ...
                  <p className="font-black text-[11px] truncate text-primary">
                    {s.val}
                  </p>
                </div>
              ))}
            ...
            <div className="space-y-4 mt-2">
              {message.text && (
                <div
                  className={`p-4 rounded-2xl text-center text-xs font-bold border animate-pulse ${
              ...
              <div className="flex flex-col gap-3">
                {isDonor ? (
                  ...
                  <button
                    ...
                    className="w-full bg-red-50 text-red-600 border border-red-200 py-4 rounded-2xl font-black text-sm hover:bg-red-100 transition-all active:scale-95 flex items-center justify-center gap-2"
                  >
                  ...
                  <button
                    onClick={handleCancelAction}
                    disabled={actionLoading}
                    className="w-full bg-orange-50 text-orange-600 border border-orange-200 py-4 rounded-2xl font-black text-sm hover:bg-orange-100 transition-all active:scale-95 flex items-center justify-center gap-2"
                  >
                    <span className="material-symbols-outlined">
                      person_remove
                    </span>{" "}
                    الانسحاب من الانتظار
                  </button>
                ) : item.status === "متاح" ? (
                  <button
                    onClick={handleRequestItem}
                    disabled={actionLoading}
                    className="w-full bg-primary text-white py-4 rounded-2xl font-black text-sm shadow-lg shadow-primary/20 hover:bg-[#004d44] transition-all active:scale-95 flex items-center justify-center gap-2"
                  >
                    <span className="material-symbols-outlined">
                  ...
                  <button
                    onClick={handleRequestItem}
                    disabled={actionLoading}
                    className="w-full bg-[#005a8c] text-white py-4 rounded-2xl font-black text-sm shadow-lg shadow-[#005a8c]/20 hover:bg-[#004a75] transition-all active:scale-95 flex items-center justify-center gap-2"
                  >
                    <span className="material-symbols-outlined">
                      notification_add
                    </span>
