...
export default function DashboardPage() {
  ...
  // --- States الخاصة بالمودالات ---
  const [showModal, setShowModal] = useState(false); // مودال OTP
  const [showRatingModal, setShowRatingModal] = useState(false); // 🟢 مودال النجوم
  ...
  const [rating, setRating] = useState(0); // 🟢 عدد النجوم المختارة
  ...
  const handleRateItem = async () => {
    if (rating === 0) return alert("الرجاء اختيار عدد النجوم ⭐");
    try {
...
<section className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
  ...
  {/* 🟢 الكوتا المتبقية - رجعناها بتصميم أنظف */}
  <div className="bg-white p-5 rounded-2xl shadow-sm border border-[#edeeef] flex flex-col justify-center text-center gap-3">
    ...
    <div className="h-2 w-full bg-[#e7e8e9] rounded-full overflow-hidden">
      {/* هون بنحسب النسبة بناءً على الكوتا الأصلية (3) */}
      <div 
  ...
  {(data.user?.quota ?? 3) > 0 
    ? `متاح لك حجز ${data.user?.quota ?? 3} أغراض إضافية` 
    : "لقد استنفدت حصتك لهذا الأسبوع ⚠️"}
</div>
  </div>
                        ...
                        <div className="flex items-center justify-center gap-2">
                          ...
                          {(item.status === "محجوز" || item.status === "تم التسليم") && (
                            <>
                              ...
                              <button onClick={() => handleReport(activeTab === "requests" ? item.donor?._id : item.bookedBy?._id, item.title)} className="text-gray-400 hover:text-red-600 p-1.5 rounded-lg"><span className="material-symbols-outlined text-sm">report</span></button>
                            </>
                          )}
                        </div>
                      </td>
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/50 backdrop-blur-sm px-4">
          <div className="bg-white rounded-3xl p-8 max-w-sm w-full shadow-2xl text-center">
            ...
            <div className="flex flex-col gap-3">
              <button onClick={handleRateItem} disabled={ratingLoading || rating === 0} className="w-full bg-[#006155] text-white font-bold py-4 rounded-2xl shadow-lg disabled:opacity-50">{ratingLoading ? "جاري الحفظ..." : "إرسال التقييم"}</button>
              <button onClick={() => { setShowRatingModal(false); setRating(0); }} className="w-full text-[#40493d] font-bold py-2 text-sm">إلغاء</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="bg-white rounded-2xl p-6 max-w-sm w-full shadow-2xl text-center">
            ...
            <form onSubmit={handleConfirmDelivery}>
              ...
              <div className="flex gap-2">
                <button type="submit" disabled={otpLoading || otp.length !== 4} className="flex-1 bg-primary text-on-primary text-sm font-bold py-3 rounded-full">{otpLoading ? "جاري التأكيد..." : "تأكيد"}</button>
